...
provider configuration, backend, Terraform state, and resource-group location.
...
graph LR
  ...
  caller --> versions[Kubernetes versions data source]
  rg --> cluster[Azure Kubernetes Service cluster]
    versions --> cluster
    subnet --> cluster
...
```hcl
module "k8s" {
  source                  = "git::https://github.com/f2calv/tf_module_azurerm_kubernetes_cluster.git//src?ref=1.0.1"
...
| location | Azure region of the parent resource group and AKS cluster. | `string` | n/a | yes |
